...
test("buildFallbackDailySummary groups materials by bucket hint", () => {
  const daily = buildFallbackDailySummary([
    ...
    { refId: 3, title: "Karpathy hints at new coding loop", source: "Andrej Karpathy", bucketHint: "ai_rumor" },
  ]);
...
test("buildFallbackDailySummary uses concise Chinese fallback copy", () => {
  ...
  assert.equal(daily.coreTech[0].title, "论文平台技术进展 1");
  ...
  assert.equal(daily.coreTech[0].summary, "该技术条目已纳入跟踪，建议通过引用原文核对方法与结论。");
