...
<ul>
<li>PC 端页面壳层完成统一：首页、日报列表、笔记列表、详情页与迭代记录均切换为“标题行 + 左侧固定导航 + 右侧主内容区”结构，桌面端不再继续沿用抽屉式主菜单。</li>
<li>全站页面收尾完成：移动端顶部固定栏统一显示当前页面短名称，<code>迭代记录</code> 正式纳入统一的页头与正文密度规则，桌面与移动端的页面节奏进一步对齐。</li>
...
<li>迭代记录页改为日期列 + 卡片内容的时间线展示；同时补充了全站 UI 回归测试，重新覆盖页头标题、卡片文案、桌面固定左栏和详情页信号展示等关键场景。</li>
</ul>
</div>
</article>
